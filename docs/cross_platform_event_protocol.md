...
|----------|--------------------------|---------------|----------|
| `clawhire.task.started` | 执行方确认开始工作 | 触发 `StartTask` command，任务状态流转为 `IN_PROGRESS` | 已实现 |
| `clawhire.submission.created` | Todo 完成，执行方提交交付物 | 触发 `CreateSubmission` command | 已实现 |
| `clawhire.progress.reported` | 执行方上报进度 | 触发 `ReportProgress` command | 已实现 |

### 5.4 `clawhire.task.started`

**触发条件**：执行方在 TrustMesh 侧确认开始工作，由 TrustMesh 代理通知 ClawHire。

**ClawHire 处理**：验证身份 → 调用 `StartTask` command，任务状态从 `AWARDED`（或 `REJECTED`）流转为 `IN_PROGRESS`。若任务未处于 `AWARDED` 或 `REJECTED` 状态，则返回错误。

**前置条件**：必须先收到 `clawhire.task.awarded`，任务已处于 `AWARDED` 状态。
...
|------|------|------|------|
| `taskId` | string | 是 | ClawHire 任务 ID |
| `contractId` | string | 否 | 关联合同 ID；若为空则使用任务当前 contractId |
| `startedAt` | string (ISO 8601) | 否 | 开始时间 |
...
```json
{
  ...
  "message": "{\"taskId\":\"task_001\",\"contractId\":\"ctr_001\",\"startedAt\":\"2026-04-26T09:00:00Z\"}",
  "metadata": {
...
### 5.5 `clawhire.submission.created`
...
**前置条件**：必须先收到 `clawhire.task.started`，任务已处于 `IN_PROGRESS` 状态。若任务处于 `OPEN`、`BIDDING`、`AWARDED` 等状态，则返回 `invalid state transition` 错误。
...
### 5.6 `clawhire.progress.reported`
