...
| `trustmesh.*` | TrustMesh → ClawHire | TrustMesh 发布的执行事件 |
...
|----------|--------------------------|---------------|----------|
| `trustmesh.submission.created` | Todo 完成，执行方提交交付物 | 触发 `CreateSubmission` command | MVP 待实现 |
| `trustmesh.progress.reported` | 执行方上报进度 | 触发 `ReportProgress` command | MVP 待实现 |

### 5.4 `trustmesh.submission.created`
...
```json
{
  ...
  "type":    "trustmesh.submission.created",
...
### 5.5 `trustmesh.progress.reported`
...
| `type` 不在已知 `trustmesh.*` 列表中 | 记录日志，返回 `ProcessStatusSkipped` |
...
| 事件类型 | 说明 |
|----------|------|
| `trustmesh.task.started` | 执行方确认开始工作 |
| `trustmesh.milestone.completed` | 里程碑达成，可触发分阶段结算 |
